...
public class ProductServiceImpl
	...
	public void deleteByBrand(Brand brand) {
		...
		products.forEach( p -> attributeRepository.deleteAllByProductId( p.getId() ) );
	...
	public Pagination<Product> search(int page, int size,
		...
		get_from_cache:
		{
			...
			List<Product> products = manager.stream()
					...
					.filter( p -> p.getAttributes().parallelStream()
							.map( Attribute::getPrice )
							.filter( price -> minPrice == null || price >= minPrice )
							.anyMatch( price -> maxPrice == null || price <= maxPrice )
					)
					.toList();
//							.anyMatch( a -> {
...
//							} )
		...
		paged.forEach( p -> getCacheManager().cache( p.getId(), p.toCacheObject() ) );
		return Pagination.of(
